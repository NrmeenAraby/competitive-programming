...
#define mid (l+r)/2
    ...
    int find(int l, int r, int node, ll k) { // same as find k-th one problem, but this time give attention to right first 
        if (l == r) {
            return l;
        }
        if (seg[R] < k) { // so go left
            return find(l, mid, L, k-seg[R]);
        }
        else {
            return find(mid + 1, r, R, k );
        }
    }
...
int main() {
    // we start with a segment tree of zeros, we update the processed idx only with one, and at eah element we query a sum from elemnt+1 to n 
