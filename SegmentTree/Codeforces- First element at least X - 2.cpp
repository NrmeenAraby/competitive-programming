...
#define mid (l+r)/2
    ...
    int findFirst_respectL(int l, int r, int node, int x,int limit) {
        if (seg[node] < x || r < limit)
            return - 1;
        if (l == r) {
            return l;
        }

        int lft= findFirst_respectL(l, mid, L, x,limit);  // if the left is below the limit, so try the right and dont just skip the right by that else
        if (lft != -1)
            return lft;

        return findFirst_respectL(mid + 1, r, R, x,limit);
...
public:
    ...
    int findFirst_respectL(int x,int limit) {
        return findFirst_respectL(0, sz - 1, 0, x,limit);
    }
    ...
    while (t--) {
        ...
        while (m--) {
            ...
            if (op == 1) {
                ...
                int x,limit;
                cin >> x>>limit;
                cout << segT.findFirst_respectL(x,limit)<<endl;
            }
        }
